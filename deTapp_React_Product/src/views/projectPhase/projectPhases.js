...
import {
  ...
  projectPhasesCreationController,
  projectPhasesupdateController,
  projectPhasesdeleteController,
} from "./controllers/projectPhaseControllers";
import FormComponent from "./components/projectPhaseComponent";
...
const Roles = () => {
  ...
  const onformSubmit = async (formData) => {
    try {
      ...
      if (isAdd) response = await projectPhasesCreationController(formData);
      else {
        formData = { ...formData, ID: selectedValue.id };
        response = await projectPhasesupdateController(formData);
      }
      ...
      openDialog(
        "warning",
        "Warning",
        `Project Phase ${isAdd ? "Addition" : "Updation"} failed`,
        {
          confirm: {
            name: "Ok",
            isNeed: true,
  ...
  const removeDataFromTable = async (selectedRow) => {
    try {
      ...
      const response = await projectPhasesdeleteController(selectedRow.id);
      ...
      if (response) {
        ...
        openDialog(
          "success",
          `Project Phase Deletion Success`,
          response.message || `Project Phase has been deleted successfully  `,
      ...
      openDialog(
        "warning",
        "Warning",
        error.errorMessage || `Project Phase Deletion failed`,
        {
          confirm: {
            name: "Ok",
            isNeed: true,
