...
const CMDPage = () => {
  ...
  const getTableData = async () => {
    try {
      startLoading();
      const response = await getCADController();
  ...
  const onformSubmit = async (formData) => {
    try {
      ...
      if (isAdd) response = await cadCreationController(formData);
      else {
        formData = {
          ...formData,
          ID: selectedValue.id,
        };
        response = await cadupdateController(formData);
      }
      ...
      if (response) {
        openDialog(
          ...
          response.message ||
            `CAD has been ${isAdd ? "addded" : "updated"} successfully`,
          {
            confirm: {
              name: "Ok",
              isNeed: true,
      ...
      openDialog(
        "warning",
        "Warning",
        `CAD ${isAdd ? "Addition" : "Updation"} failed`,
        {
          confirm: {
            name: "Ok",
            isNeed: true,
    ...
    try {
      ...
      if (response) {
        openDialog(
          ...
          response.message || `CAD has been deleted successfully  `,
          {
            confirm: {
              name: "Ok",
              isNeed: true,
    ...
    <>
      ...
      <SecondContainer className="common-table">
        <SubHeader className="table-header">
          <Typography variant="h6">
            <b>Central Adjustment Depository List</b>
          </Typography>
