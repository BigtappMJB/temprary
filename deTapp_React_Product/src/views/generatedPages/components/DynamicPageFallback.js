...
import { Box, Button, Typography, Paper, Divider, Alert, CircularProgress } from '@mui/material';
 ...
 */
const DynamicPageFallback = ({ message, showReloadButton = true, pageName = 'page' }) => {
  ...
  const extractedPageName = pageName === 'page' 
    ...
    <Container>
      <Box sx={{ py: 3, textAlign: 'center' }}>
        ...
        <Typography variant="body1" paragraph sx={{ mb: 3 }}>
          {message || `The page "${extractedPageName}" could not be loaded properly.`}
        </Typography>
        
        <Alert severity="info" sx={{ mb: 3, textAlign: 'left' }}>
          <Typography variant="body2">
            This issue occurs when a dynamic page is registered in the database, but the corresponding 
            React component file was not generated successfully by the backend.
          </Typography>
        </Alert>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, maxWidth: '400px', mx: 'auto', mt: 3 }}>
          {showReloadButton && (
            <Button 
              variant="contained" 
              color="primary" 
